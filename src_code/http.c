...
static void serve_static(int fd, char *filename, size_t filesize, sy_http_out_t *out);

static char *ROOT = NULL;

mime_type_t syeed_mime[] = 
{
    {".html", "text/html"},
    {".xml", "text/xml"},
...
const char* get_file_type(const char *type){
    ...
    for(int i = 0; syeed_mime[i].type != NULL; ++i){
        if(strcmp(type, syeed_mime[i].type) == 0)
            return syeed_mime[i].value;
    }
    // 未识别返回"text/plain"
    return "text/plain";
}
...
void do_error(int fd, char *cause, char *err_num, char *short_msg, char *long_msg){
    ...
    sprintf(body, "<html><title>SYeed Error<title>");
    sprintf(body, "%s<body bgcolor=""ffffff"">\n", body);
    sprintf(body, "%s%s : %s\n", body, err_num, short_msg);
    sprintf(body, "%s<p>%s : %s\n</p>", body, long_msg, cause);
    sprintf(body, "%s<hr><em>SYeed web server</em>\n</body></html>", body);

    // 返回错误码，组织错误响应头
    sprintf(header, "HTTP/1.1 %s %s\r\n", err_num, short_msg);
    sprintf(header, "%sServer: SYeed\r\n", header);
...
// 处理静态文件请求
void serve_static(int fd, char *filename, size_t filesize, sy_http_out_t *out){
    // 响应头缓冲（512字节）和数据缓冲（8192字节）
    char header[MAXLINE];
    char buff[SHORTLINE];
    struct tm tm;
...
int error_proess(struct stat* sbufptr, char *filename, int fd){
    ...
    if(stat(filename, sbufptr) < 0){
        do_error(fd, filename, "404", "Not Found", "SYeed can't find the file");
        return 1;
    }

    // 处理权限错误
    if(!(S_ISREG((*sbufptr).st_mode)) || !(S_IRUSR & (*sbufptr).st_mode)){
        do_error(fd, filename, "403", "Forbidden", "SYeed can't read the file");
        return 1;
    }
...
void do_request(void* ptr){
    sy_http_request_t* request = (sy_http_request_t*)ptr;
    int fd = request->fd;
    ROOT = request->root;
    ...
    while(1){
        ...
        // 分配并初始化返回数据结构
        sy_http_out_t* out = (sy_http_out_t *)malloc(sizeof(sy_http_out_t));
        sy_init_out_t(out, fd);
        ...
        sy_http_handle_header(request, out);
    ...
    // 修改已经注册描述符的事件类型
    // 重置定时器，每等待下一次请求均生效
    sy_epoll_mod(request->epoll_fd, request->fd, request, (EPOLLIN | EPOLLET | EPOLLONESHOT));
    sy_add_timer(request, TIMEOUT_DEFAULT, sy_http_close_conn);
