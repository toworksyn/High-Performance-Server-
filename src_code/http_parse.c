        ...
        switch(state){
            case sw_start:
                ...
                if(ch == CR || ch == LF)
                    break;
                if((ch < 'A' || ch > 'Z') && ch != '_')
                    return SY_HTTP_PARSE_INVALID_METHOD;
                state = sw_method;
                break;
            ...
            case sw_method:
                if(ch == ' '){
                    ...
                    switch(p - m){
                        case 3:
                            if(sy_str3_cmp(m, 'G', 'E', 'T', ' ')){
                                request->method = SY_HTTP_GET;
                                break;
                            }
                            break;
                        case 4:
                            if(sy_str3Ocmp(m, 'P', 'O', 'S', 'T')){
                                request->method = SY_HTTP_POST;
                                break;
                            }
                            if(sy_str4cmp(m, 'H', 'E', 'A', 'D')){
                                request->method = SY_HTTP_HEAD;
                                break;
                            }
                            break;
                        default:
                            request->method = SY_HTTP_UNKNOWN;
                            break;
                    }
            ...
            case sw_spaces_before_uri:
                ...
                switch(ch){
                    case ' ':
                        break;
                    default:
                        return SY_HTTP_PARSE_INVALID_REQUEST;
                }
                break;
        ...
        switch(state){
            ...
            case sw_spaces_before_colon:
                if(ch == ' ')
                    ...
                    return SY_HTTP_PARSE_INVALID_HEADER;
            ...
            case sw_cr:
                if(ch == LF){
                    state = sw_crlf;
                    hd = (sy_http_header_t *) malloc(sizeof(sy_http_header_t));
